...
System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::LoadData()
{
	try
	{
		sqlDA = gcnew SqlDataAdapter("SELECT FilmID AS Êîä, Film.Name AS Íàçâàíèå, Genre.Name AS Æàíğ, \
			yearOfRelease AS Ïğåìüåğà, FilmDirector AS Ğåæèññåğ, Availability AS Íàëè÷èå, Price AS Öåíà FROM Film	\
	...
	{
		MessageBox::Show("Îøèáêà âûâîäà äàííûõ", "Îøèáêà");
	}
}
//
...
System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::ReloadData()
{
	...
	{
		MessageBox::Show("Îøèáêà îáíîâëåíèÿ", "Îøèáêà");
	}
}
...
System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::FindButton_Click(System::Object ^ sender, System::EventArgs ^ e)
{
	try
	{
		...
		if (textBox1->Text->Length == 0) {
			MessageBox::Show("Ïîæàëóéñòà, ââåäèòå íàçâàíèå ôèëüìà", "Îøèáêà");
			return;
		}
		sqlDA = gcnew SqlDataAdapter("SELECT FilmID AS Êîä, Film.Name AS Íàçâàíèå, Genre.Name AS Æàíğ, YearOfRelease AS Ïğåìüåğà, \
			FilmDirector AS Ğåæèññåğ, Availability AS Íàëè÷èå, Price AS Öåíà FROM Film \
	...
	{
		MessageBox::Show("Îøèáêà ïîèñêà â ÁÄ", "Îøèáêà");
	}
}
...
System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::DeleteButton_Click(System::Object ^ sender, System::EventArgs ^ e)
{
	try
	{
		...
		/*if (dataGridView1->SelectedRows->Count != 1) {
			MessageBox::Show("Ïîæàëóéñòà, âûäåëèòå îäíó çàïèñü äëÿ óäàëåíèÿ", "Îøèáêà");
			return;
		}*/
	...
	{
		MessageBox::Show("Îøèáêà óäàëåíèÿ çàïèñè", "Îøèáêà");
	}
}
...
//	catch (const Exception^ ex)
//	{
//		MessageBox::Show("Îøèáêà äîáàâëåíèÿ çàïèñè\n" + e, "Îøèáêà", MessageBoxButtons::OK);
//	}
//}
...
{
	/*if (dataGridView1->SelectedRows->Count != 1) {
		MessageBox::Show("Äëÿ ğåäàêòèğîâàíèÿ âûäåëèòå îäíî ïîëå", "Îøèáêà", MessageBoxButtons::OK ,MessageBoxIcon::Error);
		return;
	}*/
...
void VideoCassetDBMetelnikov::EditAndViewDBForm::mySubscriber(System::Object ^ sender, System::EventArgs ^ e, System::String^ str)
{
	...
	{
		MessageBox::Show("Îøèáêà ìíîãîêğèòåğèàëüíîãî ïîèñêà", "Îøèáêà");
	}
...
System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::EditAndViewDBForm_FormClosing(System::Object^ sender, System::Windows::Forms::FormClosingEventArgs^ e)
{
	Application::Exit();
}

System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::âûõîäÈçÀêêàóíòàToolStripMenuItem_Click_1(System::Object^ sender, System::EventArgs^ e)
{
	LogInForStaffForm^ form = gcnew LogInForStaffForm();
	form->Show();
	this->Hide();
...
System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::èçìåíèòüÊîäÄîñòóïàToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e)
{
	EditAccessCodeForm^ form = gcnew EditAccessCodeForm();
	form->ShowDialog();
}

System::Void VideoCassetDBMetelnikov::EditAndViewDBForm::èñòîğèÿÇàêàçîâToolStripMenuItem_Click_1(System::Object^ sender, System::EventArgs^ e)
{
	HystoryOfBuyForm^ form = gcnew HystoryOfBuyForm();
	form->Show();
	this->Hide();
